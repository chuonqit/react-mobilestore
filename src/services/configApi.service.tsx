...
export const baseQueryWithReauth: BaseQueryFn<string | FetchArgs, unknown, FetchBaseQueryError> = async (args, api, extraOptions) => {
    ...
    if (result.error && (result.error as { originalStatus: number }).originalStatus === 403) {
        const profile = (api.getState() as RootState).auth;
        const responseRefresh = await baseQuery({ url: "/auth/refresh-token", method: "POST", body: { token: profile.refreshToken } }, api, extraOptions);
